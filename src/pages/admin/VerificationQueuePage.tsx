    ...
    <div className="py-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-6">
      ...
      {pendingHospitals.length === 0 ? (
        <div className="p-16 text-center bg-white rounded-2xl border border-slate-200 shadow-xs space-y-3">
          ...
          <h2 className="text-base font-bold text-slate-900">Verification Queue is Clear</h2>
        ...
        <div className="space-y-6">
          {pendingHospitals.map((hosp) => (
            <div
              key={hosp.id}
              className="bg-white rounded-2xl border-2 border-amber-200/80 p-6 shadow-sm space-y-6"
            >
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 pb-4 border-b border-slate-200">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 rounded-full bg-amber-100 text-amber-900 font-mono text-[10px] font-bold uppercase">
                      PENDING AUDIT
                    </span>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-4 py-2 rounded-xl bg-slate-100 hover:bg-rose-50 text-rose-700 hover:border-rose-300 border border-slate-200 font-bold text-xs flex items-center gap-1.5 transition-colors"
                  >
                    <XCircle className="w-4 h-4" /> Reject with Compliance Notice
                  </button>
                  <button
                    onClick={() => adminApproveHospital(hosp.id)}
                    className="px-5 py-2 rounded-xl bg-emerald-700 hover:bg-emerald-800 text-white font-bold text-xs shadow-sm flex items-center gap-1.5 transition-colors"
                  >
                    <CheckCircle2 className="w-4 h-4" /> Verify & Authorize Node
                  </button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
                ...
                <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-3">
                  ...
                  <div className="space-y-1.5">
                    <div>
                      <span className="text-slate-400 text-[10px] uppercase block">Authorized Nodal Officer:</span>
                      <strong className="text-slate-800">{hosp.officerName}</strong> ({hosp.officerDesignation})
                    </div>
                    <div>
                      <span className="text-slate-400 text-[10px] uppercase block">Direct Line & Official Email:</span>
                      <span className="font-mono text-slate-700">{hosp.phone} • {hosp.email}</span>
                    </div>
                    <div>
                      <span className="text-slate-400 text-[10px] uppercase block">Cold-Chain Infrastructure:</span>
                ...
                <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-3">
                  ...
                  <div className="space-y-2">
                    <DocItem label="Hospital Registration / NABH" filename={hosp.complianceDocs.regCertUrl} />
                    <DocItem label="Wholesale Drug License Form 20B/21B" filename={hosp.complianceDocs.drugLicenseUrl} />
                    <DocItem label="Entity GSTIN Certificate" filename={hosp.complianceDocs.gstinCertUrl} />
                    <DocItem label="Board Authorization Letter" filename={hosp.complianceDocs.boardResolutionUrl} />
                  </div>
                </div>
              </div>
            </div>
        ...
        <form onSubmit={handleConfirmReject} className="space-y-4 text-xs">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full font-medium p-3 rounded-lg border border-slate-300 focus:ring-2 focus:ring-rose-600 bg-white"
            />
          </div>
          ...
          <div className="pt-3 border-t border-slate-200 flex justify-end gap-2">
            ...
            <button
              type="submit"
              className="px-5 py-2 rounded-lg bg-rose-700 hover:bg-rose-800 text-white font-bold"
            >
              Issue Compliance Rejection Notice
            </button>
          </div>
        </form>
      </Modal>
...
const DocItem: React.FC<{ label: string; filename: string }> = ({ label, filename }) => (
  <div className="flex items-center justify-between p-2 rounded bg-white border border-slate-200 text-xs">
    <div className="overflow-hidden mr-2">
      <div className="font-semibold text-slate-800 text-[11px] truncate">{label}</div>
      <div className="font-mono text-[10px] text-slate-500 truncate">{filename}</div>
    </div>
    <span className="text-[10px] font-mono font-bold text-teal-700 px-2 py-0.5 rounded bg-teal-50 border border-teal-200 shrink-0">
      ATTACHED
    </span>
  </div>
);
